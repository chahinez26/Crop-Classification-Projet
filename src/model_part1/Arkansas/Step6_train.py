...
def plot_training(history):
    ...
    ax.set_xlabel('Epoch'); ax.set_ylabel('Loss (CrossEntropy)')
    ax.set_title('Loss courbe'); ax.legend(); ax.grid(alpha=0.3)
    ...
    ax.set_title('Accuracy courbe'); ax.legend(); ax.grid(alpha=0.3)

    plt.suptitle('MCTNet — Entraînement Arkansas (v2)', fontsize=13)
    plt.tight_layout()
    out = os.path.join(FIG_DIR, 'fig_training.png')
...
def main():
    print("=" * 55)
    print("Étape 6 — Entraînement MCTNet (v6 — LR fixe, patience=40)")
    print("=" * 55 + "\n")
    ...
    print(f"\nParamètres : {n_params:,}  (papier : 55 059)")
    diff_pct = abs(n_params - 55059) / 55059 * 100
    print(f"Écart papier : {n_params - 55059:+d} ({diff_pct:.1f}%)\n")
    ...
    print(f"\n  → Lancer maintenant : python step7_evaluate.py")
