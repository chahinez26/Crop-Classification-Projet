"""
PART 2 — ÉTAPE 4 : Entraînement des 5 configurations d'ablation — California
=============================================================================
...
Métriques cibles Part 1 (papier Table 5) :
  MCTNet baseline : OA=0.852, Kappa=0.806, F1=0.829
"""
...
def compute_metrics(model, loader, device, use_cov):
    model.eval()
    preds, trues = [], []
    with torch.no_grad():
        for X, m, y, cov in loader:
    ...
    for i in range(N_CLASSES):
        ...
        f1s.append(2*p_*r_/(p_+r_) if p_+r_ > 0 else 0.)
...
def main():
    print("="*58)
    print("Part 2 — Étape 4 : Ablation (5 configurations) — California")
    print("="*58 + "\n")
    ...
    print(f"  {'Papier MCTNet':25s}  {'0.852':>7}  {'0.806':>7}  {'0.829':>7}")
    ...
    print(f"\n✅ {res_f}\n✅ {hist_f}")
    print("\n   → Lancer : python CAL_Part2_Step5_ablation_results.py")
