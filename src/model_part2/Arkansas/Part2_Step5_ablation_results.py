...
RESULTS_FILE = r"outputs\results\Arkansas\part2\results\ablation_results.json"
HIST_FILE    = r"outputs\results\Arkansas\part2\results\ablation_histories.npz"
...
def plot_delta(res):
    ...
    ax.set_title('Gain/perte vs baseline S2-only', fontsize=12)
...
def plot_learning(res, hist):
    ...
    plt.suptitle('Courbes d\'apprentissage — 5 configurations', fontsize=13)
    plt.tight_layout()
...
def plot_radar(res):
    ...
    ax.set_title('Radar — [0.82, 1.00]', fontsize=12, pad=20)
...
def print_summary(res):
    print("\n" + "="*65)
    print("RÉSUMÉ FINAL — Part 2 : Covariables environnementales")
    print("="*65)
    print("\n  Covariables (8) :")
    ...
    print("\n" + "="*65)
    print("✅ Part 2 terminée → passer à Part 3")
    print("="*65)


def main():
    print("="*58)
    print("Part 2 — Étape 5 : Résultats d'ablation")
    print("="*58 + "\n")
    if not os.path.exists(RESULTS_FILE):
        print(f"❌ Fichier non trouvé : {RESULTS_FILE}")
        print("   Lancer d'abord : python Part2_Step4_ablation_train.py")
        return
    res, hist = load()
