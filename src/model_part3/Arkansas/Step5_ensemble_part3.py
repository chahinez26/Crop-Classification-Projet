...
def parse_args():
    ...
    parser.add_argument("--part1-tag", type=str, default="")
    parser.add_argument("--part3-tag", type=str, default="")
    return parser.parse_args()


def resolve_model_paths(part1_tag="", part3_tag=""):
    suffix1 = f"_{part1_tag}" if part1_tag else ""
    suffix3 = f"_{part3_tag}" if part3_tag else ""
    part1_model_file = os.path.join(ROOT_DIR, "model", f"best_model{suffix1}.pth")
    part3_model_file = os.path.join(ROOT_DIR, "part_3", "model", f"best_model_part3{suffix3}.pth")
    results_file = os.path.join(RESULTS_DIR, f"results_ARK_part3_ensemble{suffix1}{suffix3}.json")
...
def main():
    args = parse_args()
    part1_model_file, part3_model_file, results_file = resolve_model_paths(args.part1_tag, args.part3_tag)

    print("=" * 60)
    print("Step 5 - Ensemble Part 1 + Part 3 Arkansas")
    print("=" * 60)

    os.makedirs(RESULTS_DIR, exist_ok=True)
    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"Device: {device}")
    print(f"Part1 tag: {args.part1_tag or 'default'}")
    ...
    results = {
        "dataset": "Arkansas",
        "model": "Ensemble(Part1_MCTNet, Part3ArkansasNet)",
