"""
STEP 6 - Multi-seed ensemble for Arkansas
=========================================
Usage example:
  python Step6_multiseed_ensemble_part3.py --tags s42 s123 s777
"""
...
PART3_DATA_FILE = os.path.join(ROOT_DIR, "part_3", "npz", "ARK_part3_dataset.npz")
RESULTS_DIR = os.path.join(ROOT_DIR, "part_3", "results")
RESULTS_FILE = os.path.join(RESULTS_DIR, "results_ARK_part3_multiseed_ensemble.json")
...
def parse_args():
    ...
    parser.add_argument("--tags", nargs="+", required=True, help="Checkpoint tags from Step3/Step4, e.g. s42 s123 s777")
    parser.add_argument("--part1-tag", type=str, default="", help="Optional Part 1 checkpoint tag")
...
def load_part1_model(device, tag=""):
    suffix = f"_{tag}" if tag else ""
    ckpt = torch.load(os.path.join(ROOT_DIR, "model", f"best_model{suffix}.pth"), map_location=device)
...
def load_part3_model(device, tag):
    model_file = os.path.join(ROOT_DIR, "part_3", "model", f"best_model_part3_{tag}.pth")
...
def average_subset(probs_by_tag, subset):
    stacked = np.stack([probs_by_tag[tag] for tag in subset], axis=0)
...
def main():
    ...
    print("=" * 60)
    print("Step 6 - Multi-seed ensemble Arkansas")
    print("=" * 60)

    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"Device: {device}")
    print(f"Part1: {args.part1_tag or 'default'}")
    print(f"Tags  : {', '.join(args.tags)}\n")

    loaders = load_datasets()
    part1_model = load_part1_model(device, args.part1_tag)
    ...
    results = {
        "dataset": "Arkansas",
        "model": "MultiSeedEnsemble(Part1_MCTNet, Part3ArkansasNet)",
