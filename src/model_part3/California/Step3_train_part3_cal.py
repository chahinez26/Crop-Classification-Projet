...
from Step2_model_part3_cal import Part3CaliforniaNet, count_parameters
...
MODEL_DIR = os.path.join(ROOT_DIR, "California", "part_3", "model")
NPZ_DIR = os.path.join(ROOT_DIR, "California", "part_3", "npz")
FIG_DIR = os.path.join(ROOT_DIR, "California", "part_3", "figures")
...
def resolve_paths(tag):
    ...
    model_file = os.path.join(MODEL_DIR, f"best_model_part3_cal{suffix}.pth")
    history_file = os.path.join(NPZ_DIR, f"training_history_part3_cal{suffix}.npz")
    figure_file = os.path.join(FIG_DIR, f"fig_training_part3_cal{suffix}.png")
...
def plot_training(history, figure_file):
    ...
    axes[1].plot(epochs, np.array(history["train_acc"]) * 100, label="Train", color="#1f77b4")
    axes[1].plot(epochs, np.array(history["val_acc"]) * 100, label="Val", color="#d62728")
    axes[1].set_title("Accuracy")
    axes[1].grid(alpha=0.3)
    axes[1].legend()

    axes[2].plot(epochs, np.array(history["train_f1"]) * 100, label="Train", color="#1f77b4")
    axes[2].plot(epochs, np.array(history["val_f1"]) * 100, label="Val", color="#d62728")
    axes[2].set_title("Macro-F1")
    axes[2].grid(alpha=0.3)
    axes[2].legend()

    plt.suptitle("Part 3 California - training", fontsize=13)
    plt.tight_layout()
...
def main():
    ...
    print("=" * 60)
    print("Step 3 - Train Part 3 California model")
    print("=" * 60)
    ...
    for epoch in range(1, EPOCHS + 1):
        t0 = time.time()
        train_loss, train_acc, train_f1 = run_epoch(model, train_loader, criterion, optimizer, device, training=True)
        val_loss, val_acc, val_f1 = run_epoch(model, val_loader, criterion, optimizer, device, training=False)
    ...
    print(f"Best epoch: {best_epoch} | best score: {best_score:.4f} | best val loss: {best_val_loss:.4f}")

    np.savez(history_file, **{k: np.array(v) for k, v in history.items()})
    print(f"Saved: {history_file}")
    plot_training(history, figure_file)
    print("\nNext step: run Step4_evaluate_part3_cal.py")
