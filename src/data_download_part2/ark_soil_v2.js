// =============================================================================
// MCTNet GEE — SOIL COVARIATES v2 — Part 2
// CORRECTION : unmask() ajouté + ordre addBands sécurisé
//
// POURQUOI v1 pouvait perdre des points :
//   OpenLandMap résolution native = 250m.
//   Certains pixels cropland CDL à 30m tombent en bordure de tuiles
//   OpenLandMap → valeur null → dropNulls=True → point perdu → < 5000.
//   Résultat : distribution des classes incorrecte → biais dans le modèle.
//
// CORRECTION :
//   .unmask(-1) sur chaque image sol → aucun pixel null → dropNulls safe
//   Les rares valeurs -1 (< 0.1% attendu pour Arkansas) seront identifiées
//   et imputées par la médiane de classe dans Step9_merge_covariates.py.
//
// Variables (identiques à v1, logique correcte) :
...
//   On supprime les .divide() ici et on laisse les valeurs brutes.
//   Step9_merge_covariates.py s'occupe des conversions et normalisation.
//   Cela évite les erreurs de type float dans stratifiedSample.
//
// PROCÉDURE (2 runs) :
//   ZONE_INDEX = 0 → ARK_SOIL_Z0.csv
//   ZONE_INDEX = 1 → ARK_SOIL_Z1.csv
// =============================================================================
